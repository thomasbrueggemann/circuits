...
bool CircuitsAudioProcessor::acceptsMidi() const { return false; }

bool CircuitsAudioProcessor::producesMidi() const { return false; }

bool CircuitsAudioProcessor::isMidiEffect() const { return false; }

double CircuitsAudioProcessor::getTailLengthSeconds() const { return 0.0; }

int CircuitsAudioProcessor::getNumPrograms() { return 1; }

int CircuitsAudioProcessor::getCurrentProgram() { return 0; }
...
void CircuitsAudioProcessor::changeProgramName(int index,
                                               const juce::String &newName) {
  juce::ignoreUnused(index, newName);
}

void CircuitsAudioProcessor::prepareToPlay(double sampleRate,
                                           int samplesPerBlock) {
...
bool CircuitsAudioProcessor::isBusesLayoutSupported(
...
void CircuitsAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer,
...

  {
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
      ...
      if (probeNodeId >= 0) {
        voltageHistory[historyIndex] =
            static_cast<float>(circuitEngine.getNodeVoltage(probeNodeId));
...
bool CircuitsAudioProcessor::hasEditor() const { return true; }
...
void CircuitsAudioProcessor::setStateInformation(const void *data,
                                                 int sizeInBytes) {
  juce::String json(static_cast<const char *>(data),
...
void CircuitsAudioProcessor::getLatestSamples(std::vector<float> &dest) const {
  const juce::ScopedLock sl(historyLock);
  dest.resize(HISTORY_SIZE);

  for (size_t i = 0; i < HISTORY_SIZE; ++i) {
    dest[i] = voltageHistory[(historyIndex + i) % HISTORY_SIZE];
  }
}

void CircuitsAudioProcessor::setProbeNode(int nodeId) { probeNodeId = nodeId; }
